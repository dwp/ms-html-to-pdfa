...
public class HtmlToPdfGenerator {
    ...
    public String createPdfaDocument(String html, String colourProfile, Map<String, String> fontMap, String conformanceLevel) throws PdfaGeneratorException {
        try {
            String base64pdf;
            ...
            try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
                ...
                if (useConformanceLevelParameter(conformanceLevel)) {
                    ...
                    pdfBuilder.usePdfAConformance(PdfRendererBuilder.PdfAConformance.valueOf(conformanceLevel));
    ...
    private boolean useConformanceLevelParameter(String conformanceLevel) throws PdfaGeneratorException {
        ...
        try {
            ...
            if ((conformanceLevel == null) || (!conformanceLevel.equalsIgnoreCase(PdfExtendedConstants.PDF_UA_CONFORMANCE))) {
                throw new PdfaGeneratorException(String.format("'%s' is not a valid conformance level, aborting", conformanceLevel));
            }
        }
    ...
    private void verifyFontApplication(Map<String, String> fontMap, String html, String conformanceLevel) throws PdfaGeneratorException {
        if ((html != null) && (!conformanceLevel.equalsIgnoreCase(PdfRendererBuilder.PdfAConformance.NONE.toString()))) {
            LOGGER.debug("validate that all fonts in the document are contained in the font map");

            for (String fontHtml : html.lines().filter(p -> p.contains("font-family")).collect(Collectors.toList())) {
                ...
                if (fontMissing) {
                    throw new PdfaGeneratorException(String.format("html element requests %s.  It is not passed in the font map, cannot encode.", fontHtml.replaceAll(";", "").trim()));
                }
            }
        }
    }
